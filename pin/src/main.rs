...
/// Add the `get-tags` sub-command to the [`Command`]
fn add_get_tags(app: Command) -> Command {
    app.subcommand(
    Command::new("get-tags")
...
/// Add the `send` sub-command to the [`Command`]
fn add_send(app: Command) -> Command {
    app.subcommand(
        Command::new("send")
            ...
            .arg(
                Arg::new("tag")
                    ...
                    .value_parser(value_parser!(String))
                    .num_args(1)
                    .action(ArgAction::Append), // "-t a -t b...", not "-t a b..."
            )
            .arg(
                Arg::new("target")
                    .short('r')
...
/// Add the `delete` sub-command to the [`Command`]
fn add_delete(app: Command) -> Command {
    app.subcommand(
        Command::new("delete")
...
/// Add the `rename-tag` sub-command to the [`Command`]
fn add_rename_tag(app: Command) -> Command {
    app.subcommand(
...
fn make_app(home: Option<PathBuf>) -> Command {
    let mut cfg_arg = Arg::new("config")
        ...
        .value_name("FILE");
    if let Some(mut pathb) = home {
        pathb.push(".pin");
        cfg_arg = cfg_arg.default_value(pathb.into_os_string());
    }
...
async fn send_tags(sub: &ArgMatches, cfg: Config, client: Client) -> Result<()> {
    ...
    let read_later = read_later || sub.get_flag("read-later");

    let insty = if sub.get_flag("instapaper") {
...
async fn main() -> Result<()> {
    ...
    // Build up the `Command` object...
    let mut app = make_app(home);
    ...
    let cfg: Config = match matches.get_one::<PathBuf>("config") {
        Some(cfg_path) => {
    ...
    if let Some(sub) = matches.subcommand_matches("get-tags") {
        let alpha = sub.get_flag("alphabetical");
        let desc = sub.get_flag("descending");
...
mod test {
    ...
    async fn test_ignore_blanks() {
        ...
        // message
        let dot_pin = home::home_dir();
        let mut app = make_app(dot_pin);
        app = add_send(app);

        let cfg = Config::default();
        let mut server = mockito::Server::new();
        let client = Client::new(&server.url(), "sp1ff:baadf00d").unwrap();
        ...
        let mock = server
            .mock("GET", Matcher::Regex(r"^/v1/posts/add.*".to_string()))
            .with_status(201)
            .expect(2)
            .create();
